...
DB_REMOTE_ROOT_HOST=${DB_REMOTE_HOST:-"172.17.0.1"}
...
initialize_mysql_database() {
  ...
  if [ ! -d ${MYSQL_DATA_DIR}/mysql ]; then
    ...
    # Resolve sysPass webserver hostname
    if [ "${DB_REMOTE_HOST}" == "syspass-app" ]; then
      DB_REMOTE_HOST=`ping -c 1 -q syspass-app | head -n 1 | awk '{print $3}' | sed -e 's/.*(\(.*\)):/\1/g'`
    fi

    if [ -n "${DB_REMOTE_ROOT_NAME}" -a -n "${DB_REMOTE_HOST}" ]; then
      echo "Creating remote user \"${DB_REMOTE_ROOT_NAME}\" with root privileges..."
      mysql -uroot \
      -e "GRANT ALL PRIVILEGES ON *.* TO '${DB_REMOTE_ROOT_NAME}'@'${DB_REMOTE_HOST}' IDENTIFIED BY '${DB_REMOTE_ROOT_PASS}' WITH GRANT OPTION; FLUSH PRIVILEGES;"
    fi
